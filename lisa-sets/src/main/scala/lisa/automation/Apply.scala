...
class Apply(using val lib: Library, val proof: lib.Proof)(thm: proof.Fact) extends ProofTactic {
  ...
  private def searchPremises(
      seq: Sequent,
      f: Formula,
      takenTVars: Iterable[Variable] = Iterable.empty,
      takenFVars: Iterable[VariableFormula] = Iterable.empty
      ...
      TacticSubproof { sp ?=>
        ...
        matchFormula(botCcl, thmCcl) match
          case Some((formulaCclAssignment, termCclAssignment)) =>
                  ...
                  case Some(updatedThmAfterCclUnification, premAfterCclUnification) =>
                    ...
                    val updatedThmAfterWeakening: sp.Fact =
                      premAfterCclUnification.statement.left.foldLeft(updatedThmAfterCclUnification)((updatedThmDuringWeakening, premPrem) => {
                        if botNormalized.left.contains(premPrem) then lib.have(updatedThmDuringWeakening.statement +<< premPrem) by Weakening(updatedThmDuringWeakening)
                        else
                    ...
                    if cutStep.isValid then lib.have(cutStep)
                    else break(proof.InvalidProofTactic(s"The following argument could not be applied to the theorem\n${premAfterCclUnification.statement}"))
            ...
            if finalStep.isValid then lib.have(finalStep)
            else break(proof.InvalidProofTactic(s"The given theorem could not prove the sequent\n. Deduced theorem: ${thmAfterLastUnification.statement}"))
