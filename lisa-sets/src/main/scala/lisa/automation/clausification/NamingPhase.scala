...
private[clausification] object NamingPhase:
   ...
   */
  private final case class Site(subst: Expression, phiBody: Expression, p: Variable, pol: Int, xs: Seq[Variable], rebuild: Expression => Expression)
  ...
  def certifyNaming(problem: Problem, prover: ClausificationProver, goal: Set[Int] = Set.empty)(using o: ClausifierOptions): ClausificationProof =
    ...
    val allData: IndexedSeq[HypData] = problem.hypotheses.toIndexedSeq.map { hyp =>
      ...
      HypData(phi, Iterator.unfold(phi)(current => nameOne(current, counter, threshold, markers, problem.frozen).map(s => (s, s.named))).toIndexedSeq)
    }
    ...
    dischargeAssumptionsLatestFirst(
      ...
      { j =>
        val st = flatSteps(j)
        (Map(st.nm -> NamingSupport.lambdifyAll(st.subst, st.freeVars)), quantifieds(j), NamingSupport.proveQuantifiedReflIff(st.subst, st.freeVars))
      }
    )
